  ...
  return (
    <div className="mt-2 rounded-md border border-slate-700 bg-slate-800/70 p-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-2 gap-x-6">
        <Row label="Logo PNG detected" ok={pngLogo} extra={pngLogo ? 'PNG in use' : 'Fallback SVG in use'} />
        <Row label="Backend health" ok={backendOk} extra={BACKEND_URL} />
        <Row label="Auth token present" ok={authOk} />
        <Row label="Projects API reachable" ok={projectsOk} />
        <Row label="Chat endpoint reachable" ok={chatOk} extra={chatOk ? 'OK' : devMode ? 'Dev fallback available' : 'Unreachable'} />
        <Row label="Dev mode (fallbacks enabled)" ok={devMode} />
      </div>
      {notes.length > 0 ? (
